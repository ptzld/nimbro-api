...
from ..base.transcriptions_base import TranscriptionsBase

default_settings = {
    'logger_severity': None,
    'logger_name': "Transcriptions",
    'endpoints': {
        'OpenAI': {
        ...
        'AIS': {
            'api_url': "https://api-code.ais.uni-bonn.de/v1/audio/transcriptions",
...
class Transcriptions(Client):
    ...
    def __init__(self, settings=None, **kwargs):
        """
        Create a client implementing OpenAI's v1 Transcriptions API (https://developers.openai.com/api/reference/resources/audio/subresources/transcriptions/methods/create).
    ...
    def get_settings(self, name=None):
        ...
        Settings:
            ...
            input_language (str):
                The language of the audio input as ISO-639-1, or an empty string to auto-detect the language.
            temperature (float | int):
                Sampling temperature for the transcription process (from 0.0 to 1.0).
            prompt (str):
                Guide the model's style using the audio language, or use an empty string for default behavior.
            response_format (str):
                Response format in ["json", "verbose_json", "diarized_json", "text", "srt", "vtt"].
    ...
    def get_transcription(self, audio, **kwargs):
        """
        Request an audio transcription.
        ...
        Returns:
            tuple[bool, str, dict | str | None]: A tuple containing:
                - bool: `True` if the operation succeeded, `False` otherwise.
                - str: A descriptive message about the operation result.
                - dict | str | None: The transcription result according to the set 'response_format', or `None` if not successful.
        """
        return self._base.wrap(1, self._base.get_transcription, audio, **kwargs)
