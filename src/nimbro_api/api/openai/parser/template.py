...
# When requesting one choice, `completion` is the complete result dictionary.
# When requesting multiple choices, the parser is executed independently for every choice and `completion` is the respective choice dictionary.
# When setting `success` to `False`, the response remains eligible for retry according to the 'retry' setting.
# To prevent retry, raise an `UnrecoverableError` instead (`from nimbro_api.utility.misc import UnrecoverableError`).
...
def parse(self, success, message, completion):
    ...
    if 'tools' in completion:
        assert isinstance(completion['tools'], list)
        assert len(completion['tools']) > 0
        for tool_call in completion['tools']:
    ...
    # - Value of `completion` must be of type `dict`.
    # - Value of `completion['logs']` must be of type `list`.
    ...
    # - When requesting multiple choices, modifications affect only the respective choice, while setting `success` to `False` fails the complete response.
