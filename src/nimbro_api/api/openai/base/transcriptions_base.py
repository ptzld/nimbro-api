...
class TranscriptionsBase(ClientBase):
    ...
    def set_settings(self, settings, mode="set"):
        ...
        # input_language
        assert_type_value(obj=settings['input_language'], type_or_value=str, name="setting 'input_language'")
    ...
    def get_transcription(self, audio):
        ...
        if self._settings['input_language'] != "":
            data['language'] = self._settings['input_language']
        if self._settings['prompt'] != "":
            data['prompt'] = self._settings['prompt']

        # use API
        success, message, transcription = post_request(
            api_name="Transcriptions API",
        ...
        if success:
            ...
            if isinstance(transcription, str):
                ...
                try:
                    assert_type_value(obj=transcription, type_or_value=dict, name="transcription response")
                    assert_log(expression='text' in transcription, message=f"Expected transcription response to contain the key 'text': {format_obj(transcription)}")
                    assert_type_value(obj=transcription['text'], type_or_value=str, name="key 'text' in transcription response")
        ...
        if success:
            if audio_path is None:
                message = f"Transcribed audio in '{time.perf_counter() - stamp:.3f}s': '{transcription_str}'"
            else:
                message = f"Transcribed audio '{audio_path}' in '{time.perf_counter() - stamp:.3f}s': '{transcription_str}'"
        else:
            transcription = None
