...
from ..base.dam_base import DamBase
...
default_settings = {
    ...
    'endpoints': {
        'AIS': {
            'api_url': "https://api-code.ais.uni-bonn.de/v1/vision/dam",
            'key_type': "environment",
            'key_value': "AIS_API_KEY"
        },
        'localhost': {
            'api_url': "http://localhost:9037",
            'key_type': "environment",
...
class Dam(Client):
    """
    This is an implementation of the NimbRo Vision Servers API for the Describe Anything Model (https://github.com/AIS-Bonn/nimbro_vision_servers/tree/main/models/dam),
    ...
    def get_settings(self, name=None):
        ...
        Settings:
            ...
            endpoints (dict[dict]):
                Endpoint definitions pointing to providers of the NimbRo Vision Servers API for the Describe Anything Model.
            ...
            validate_flavor (float | int | bool):
                Validate the set 'flavor' is available with the set 'endpoint' when using `load()` or `get_descriptions()`:
                - Use `float` or `int` to set the time in seconds permitted for cached responses to be reused before requesting a new one.
                - Use `True` to force validation, or `False` to deactivate it.
            flavor (str):
                Name of the model flavor used (see `get_flavors()`).
            query (str):
                Model query parameter providing the task.
            temperature (float | int):
                Model temperature parameter (from 0.0 to 1.0).
            top_p (float | int):
                Model top_p parameter (from 0.0 to 1.0).
            num_beams (int):
    ...
    def get_descriptions(self, image, prompts, **kwargs):
        """
        Request descriptions for prompts of an image.
        ...
        Returns:
            tuple[bool, str, list[str] | None]: A tuple containing:
                - bool: `True` if the operation succeeded, `False` otherwise.
                - str: A descriptive message about the operation result.
                - list[str] | None: A list (`list`) of descriptions (`str`) corresponding to `prompts`, or `None` if not successful.
        """
        return self._base.wrap(1, self._base.get_descriptions, image, prompts, **kwargs)
