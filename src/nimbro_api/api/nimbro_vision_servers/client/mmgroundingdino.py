...
from ..base.mmgroundingdino_base import MmGroundingDinoBase

default_settings = {
    'logger_severity': None,
    'logger_name': "MMGroundingDINO",
    'message_results': True,
    'endpoints': {
        'AIS': {
            'api_url': "https://api-code.ais.uni-bonn.de/v1/vision/mmgroundingdino",
            'key_type': "environment",
            'key_value': "AIS_API_KEY"
        },
        'localhost': {
            'api_url': "http://localhost:9035",
            'key_type': "environment",
...
class MmGroundingDino(Client):
    """
    This is an implementation of the NimbRo Vision Servers API for MM-Grounding-DINO and LLMDet (https://github.com/AIS-Bonn/nimbro_vision_servers/tree/main/models/mmgroundingdino),
    ...
    def get_settings(self, name=None):
        ...
        Settings:
            ...
            endpoints (dict[dict]):
                Endpoint definitions pointing to providers of the NimbRo Vision Servers API for MM-Grounding-DINO and LLMDet.
            ...
            validate_flavor (float | int | bool):
                Validate the set 'flavor' is available with the set 'endpoint' when using `load()` or `get_detections()`:
                - Use `float` or `int` to set the time in seconds permitted for cached responses to be reused before requesting a new one.
                - Use `True` to force validation, or `False` to deactivate it.
            flavor (str):
                Name of the model flavor used (see `get_flavors()`).
            min_confidence (float):
                Confidence threshold below which detections are discorded. Confidences are between 0.0 and 1.0.
            nms_iou (float | None):
    ...
    def get_detections(self, image, prompts, **kwargs):
        """
        Request the detection of prompts for an image.
