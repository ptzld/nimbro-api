...
from ..base.sam2_realtime_base import Sam2RealtimeBase

default_settings = {
    'logger_severity': None,
    'logger_name': "SAM2Realtime",
    'endpoints': {
        'AIS': {
            'api_url': "https://api-code.ais.uni-bonn.de/v1/vision/sam2_realtime",
            'key_type': "environment",
            'key_value': "AIS_API_KEY"
        },
        'localhost': {
            'api_url': "http://localhost:9036",
            'key_type': "environment",
...
class Sam2Realtime(Client):
    """
    This is an implementation of the NimbRo Vision Servers API for the Segment Anything Model 2 real-time (https://github.com/AIS-Bonn/nimbro_vision_servers/tree/main/models/sam2_realtime),
    ...
    def get_settings(self, name=None):
        ...
        Settings:
            ...
            endpoints (dict[dict]):
                Endpoint definitions pointing to providers of the NimbRo Vision Servers API for the Segment Anything Model 2 real-time.
            ...
            validate_status (float | int | bool):
                Validate the set 'flavor' is available with the set 'endpoint' when using `load()` or `get_response()`
                and validate the tracker is initialized when tracking with `get_response()`:
                - Use `float` or `int` to set the time in seconds permitted for cached responses to be reused before requesting a new one.
                - Use `True` to force validation, or `False` to deactivate it.
            flavor (str):
    ...
    def get_status(self, age=0, **kwargs):
        ...
        Returns:
            tuple[bool, str, str | None, bool | None]: A tuple containing:
                - bool: `True` if the operation succeeded, `False` otherwise.
                - str: A descriptive message about the operation result.
                - str | None: The name of the model flavor loaded by the 'endpoint', or `None` if either not successful, or no model is loaded.
                - bool | None: `True` if the tracker loaded by the 'endpoint' is initialized, or `None` if either not successful, or no model is loaded.
        """
        return self._base.wrap(2, self._base.get_status, age, **kwargs)
    ...
    def get_response(self, image, prompts=None, **kwargs):
        """
        Either request the initialization of tracks with prompts for an image or continue existing tracks with a newer image.
